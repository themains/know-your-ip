...
class TestPackage:
    ...
    def test_imports(self):
        ...
        from know_your_ip import load_config, clean_colname
        assert callable(load_config)
        assert callable(clean_colname)
        
    def test_all_exports(self):
        """Test that all expected functions are exported."""
        expected_exports = [
            'load_config', 'maxmind_geocode_ip', 'geonames_timezone',
            'tzwhere_timezone', 'ipvoid_scan', 'abuseipdb_web',
            'abuseipdb_api', 'censys_api', 'shodan_api', 'virustotal_api',
            'ping', 'traceroute', 'query_ip', 'apivoid_api'
        ]
